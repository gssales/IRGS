...
def generate_path(viewpoint_cameras, n_frames=480):
  ...
  for c2w in new_poses:
      ...
      cam.R = np.transpose(w2c[:3,:3])  # R is stored transposed due to 'glm' in CUDA code
      cam.T = w2c[:3, 3]
      ...
      cam.R = torch.tensor(cam.R, dtype=torch.float32, device='cuda')
      cam.T = torch.tensor(cam.T, dtype=torch.float32, device='cuda')

      v, u = torch.meshgrid(torch.arange(cam.image_height, device='cuda'),
                            torch.arange(cam.image_width, device='cuda'), indexing="ij")
      focal_x = cam.image_width / (2 * np.tan(cam.FoVx * 0.5))
      focal_y = cam.image_height / (2 * np.tan(cam.FoVy * 0.5))
      rays_d_camera = torch.stack([(u - cam.image_width / 2 + 0.5) / focal_x,
                                (v - cam.image_height / 2 + 0.5) / focal_y,
                                torch.ones_like(u)], dim=-1).reshape(-1, 3)
      rays_d = rays_d_camera @ cam.world_view_transform[:3, :3].T
      cam.rays_d = F.normalize(rays_d, dim=-1)
      cam.rays_d_hw = cam.rays_d.reshape(cam.image_height, cam.image_width, 3)
      cam.rays_d_hw_unnormalized = rays_d.reshape(cam.image_height, cam.image_width, 3)
...
if __name__ == "__main__":
    ...
    print("render videos ...")
    traj_dir = os.path.join(args.model_path, 'traj', "ours_{}_ellipse".format(scene.loaded_iter))
